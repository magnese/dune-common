...
template<typename M>
void printMatrix(M& m){
  #ifdef DEBUG_FLAG
  #if DEBUG_FLAG
  for(typename M::iterator rowIt=m.begin();rowIt!=m.end();++rowIt) printVector(*rowIt);
  std::cout<<std::endl;
  #endif
  #endif
}
...
int main(int argc, char** argv){
  ...
  const size_t numColors(2); // 0 when tid is even, 1 when tid is odd
